...
// Singleton instance for the application (lazy initialization)
// The client is only instantiated when actually called at runtime,
// not during Next.js build (which collects page data for API routes)
let _vertexAIClient: VertexAIClient | null = null;

export function getVertexAIClient(): VertexAIClient {
  if (!_vertexAIClient) {
    _vertexAIClient = new VertexAIClient();
  }
  return _vertexAIClient;
}

// Deprecated: use getVertexAIClient() instead
// Kept for backward compatibility but not auto-initialized
export const vertexAIClient = {
  generateContent: (...args: Parameters<InstanceType<typeof VertexAIClient>['generateContent']>) =>
    getVertexAIClient().generateContent(...args),
  generateStructuredContent: (...args: Parameters<InstanceType<typeof VertexAIClient>['generateStructuredContent']>) =>
    getVertexAIClient().generateStructuredContent(...args),
  generateContentWithRetry: (...args: Parameters<InstanceType<typeof VertexAIClient>['generateContentWithRetry']>) =>
    getVertexAIClient().generateContentWithRetry(...args),
  batchGenerate: (...args: Parameters<InstanceType<typeof VertexAIClient>['batchGenerate']>) =>
    getVertexAIClient().batchGenerate(...args),
  getConfig: () => getVertexAIClient().getConfig(),
  updateConfig: (...args: Parameters<InstanceType<typeof VertexAIClient>['updateConfig']>) =>
    getVertexAIClient().updateConfig(...args),
};
