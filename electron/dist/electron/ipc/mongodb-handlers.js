...
const USER_URI_FILE_PATH = path.join(electron_2.app.getPath('userData'), 'healthtrack-settings.json');
/**
...
function setupMongoDBIpcHandlers() {
        ...
        try {
            ...
            if (connectionInfo.length > 0) {
                try {
                    ...
                    const collectionsInfo = await dataSourceManager.executeActiveSourceQuery({
                        type: 'raw',
                        params: {},
                        rawQuery: async (db) => {
    ...
    electron_1.ipcMain.handle('db-findOne', async (event, collection, query) => {
        try {
            ...
            try {
                const result = await dataSourceManager.executeActiveSourceQuery({
    ...
    electron_1.ipcMain.handle('db-testConnection', async (event, uri) => {
        try {
