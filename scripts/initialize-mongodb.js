 ...
 * Runs in the main process context and directly accesses DataSourceManager.
 */
const fs = require('fs');
...
async function initializeDatabasesSwitchboard() {
  ...
  try {
    // For Electron, we need to communicate with the DataSourceManager
    // This script will be called from the main process context where Switchboard is available
    ...
    if (userDbUri) {
      ...
      console.log('🎯 [DBINIT] User database initialization will be handled by UI onboarding');
    } else {
      console.log('⚠️ [DBINIT] No user database URI in settings - user will need to configure via UI');
    }
    
    console.log('🎯 [DBINIT] Database URIs checked. Initialization delegated to Switchboard.');
    return true;
